...
subtitle: "Lesson 1.6: Properties of the OLS Estimator"
format:
    html:
        page-layout: article
        grid:
        ...
        toc-location: left
        toc-expand: true
        toc-title: "Lesson 1.6: Properties of the OLS Estimator"
...
In the last two lessons, we discussed the OLS estimator for linear regressions. But we have to ask ourselves - is OLS a good estimator? Why should we use OLS? In this lesson, we focus on the properties of OLS, and the conditions under which OLS is a good estimator.
...
For multiple regression OLS estimates of $\hat\beta_0 , \dots \hat\beta_k$ are chosen to satisfy the following first order conditions (see [1.6.4](https://statsnotes.github.io/theory/6.html#mathematics-of-the-ordinary-least-squares-estimator)):
...
-   **MLR.4 (Zero Conditional Mean)**. Mathematically: $E(u|x_1, \dots, x_k) = 0$.
:::
...
This means that the parameters of the model $\beta_0$ and $\beta_1$ must not be multiplied together - they must be added together. For example, the following is linear:
...
-   This is also a consideration in categorical explanatory variables, which we will cover in lesson 1.8
...
This is the [key assumption]{.underline} of OLS, since it is the one that is most frequently violated. We will discuss this in more detail in section 1.6.5.
...
-   The most critical assumption, and the one most frequently violated, is SLR.4 Zero-Conditional Mean.
...
# 1.6.4: Proof of OLS Unbiasedness in Simple Linear Regression
...
-   Do not worry, we will not care about this too much. The reason is because of asymptotic properties of OLS, which allow us to weaken Zero-Conditional Mean to just exogeneity $Cov(x_j, u) = 0$ (beyond the scope of this class).
:::
...
These moments also imply that $Cov(x,u) = 0$, which was proved in 1.6.1. This is the assumption of exogeneity, as discussed in 1.6.5
...
The Ordinary Least Squares Regression line also is the best approximation of the conditional expectation function $E(y|x)$, [assuming the conditional expectation function is linear]{.underline}. That means, we can view a regression as also a conditional expectation function.
...
What we want to prove is that the OLS estimates $\hat\beta_0$ and $\hat\beta_1$ best estimate the parameters $b_0$ and $b_1$ of the Conditional Expectation Function, which means that if true, OLS is the best approximation of the conditional expectation function.
...
These are equivalent to the OLS Method of Moments estimator conditions, so the same estimates will be produced.
...
-   We know that the expecation of a random variable is the best guess of the random variable's value (as explained in [1.1.4](https://statsnotes.github.io/theory/1.html#expected-value-and-mean)).
